...
def process(data_dir=base_collections_path, user_pip=None, user_bindep=None):
    paths = []
    path_root = os.path.join(data_dir, 'distronode_collections')
...
class CollectionDefinition:
    ...
    def target_dir(self):
        ...
        return os.path.join(
            base_collections_path, 'distronode_collections',
            namespace, name
        )
...
def parse_args(args=None):
    ...
    parser = argparse.ArgumentParser(
        prog='introspect',
        description=(
            'distronode-builder introspection; injected and used during execution environment build'
        )
    )
...
def create_introspect_parser(parser):
    ...
    introspect_parser.add_argument(
        ...
        help=(
            'Distronode collections path(s) to introspect. '
            'This should have a folder named distronode_collections inside of it.'
        )
    )
...
EXCLUDE_REQUIREMENTS = frozenset((
    ...
    'distronode', 'distronode-base', 'python', 'distronode-core',
    # general python test requirements
    'tox', 'pycodestyle', 'yamllint', 'pylint',
    'flake8', 'pytest', 'pytest-xdist', 'coverage', 'mock', 'testinfra',
    # test requirements highly specific to Distronode testing
    'distronode-lint', 'molecule', 'galaxy-importer', 'voluptuous',
