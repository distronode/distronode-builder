...
class Containerfile:
    ...
    def __init__(self,
        ...
        :param str galaxy_keyring: GPG keyring file used by distronode-galaxy to opportunistically
            validate collection signatures.
        :param int galaxy_required_valid_signature_count: Number of sigs (prepend + to disallow no sig)
            required for distronode-galaxy to accept collections.
    ...
    def prepare(self) -> None:
        ...
        if not self.definition.builder_image:
            ...
            if self.definition.distronode_ref_install_list:
                self.steps.append('RUN $PYCMD -m pip install --no-cache-dir $DISTRONODE_INSTALL_REFS')
        ...
        if self.definition.get_dep_abs_path('galaxy'):
            ...
            # Run the check for the 'distronode-galaxy' executable. This will fail
            # the build if the command is not found.
            self.steps.append("RUN /output/scripts/check_galaxy")

            self._prepare_distronode_config_file()
        ...
        # Run the check for 'distronode' and 'distronode-runner' installations for
        # any EE version 3 or above, unless explicitly skipped.
        if self.definition.version >= 3 and not self.definition.options['skip_distronode_check']:
            self.steps.append("RUN /output/scripts/check_distronode $PYCMD")
    ...
    def _insert_global_args(self, include_values: bool = False) -> None:
        ...
        global_args = {
            ...
            'DISTRONODE_GALAXY_CLI_COLLECTION_OPTS':
                self.definition.build_arg_defaults['DISTRONODE_GALAXY_CLI_COLLECTION_OPTS'],
            'DISTRONODE_GALAXY_CLI_ROLE_OPTS': self.definition.build_arg_defaults['DISTRONODE_GALAXY_CLI_ROLE_OPTS'],
            'DISTRONODE_INSTALL_REFS': self.definition.distronode_ref_install_list,
        }
    ...
    def _create_folder_copy_files(self) -> None:
        ...
        if self.definition.distronode_config:
            copy_file(
                self.definition.distronode_config,
                os.path.join(self.build_outputs_dir, 'distronode.cfg')
            )

        # HACK: this sucks
        scriptres = importlib.resources.files('distronode_builder._target_scripts')
        script_files = (
            'assemble', 'install-from-bindep', 'introspect.py', 'check_galaxy',
            'check_distronode', 'entrypoint'
        )
    ...
    def _prepare_distronode_config_file(self) -> None:
        if self.definition.version != 1:
            return

        distronode_config_file_path = self.definition.distronode_config
        if distronode_config_file_path:
            context_file_path = os.path.join(
                constants.user_content_subfolder, 'distronode.cfg')
            self.steps.extend([
                f"COPY {context_file_path} ~/.distronode.cfg",
                "",
            ])
    ...
    def _prepare_label_steps(self) -> None:
        self.steps.extend([
            "LABEL distronode-execution-environment=true",
        ])
    ...
    def _prepare_galaxy_install_steps(self) -> None:
        ...
        if self.original_galaxy_keyring:
            ...
            # verification because older versions (<2.13) of distronode-galaxy do
            ...
            # in the final image.
            env = "DISTRONODE_GALAXY_DISABLE_GPG_VERIFY=1 "

        self.steps.append(
            f"RUN distronode-galaxy role install $DISTRONODE_GALAXY_CLI_ROLE_OPTS "
            f"-r {constants.CONTEXT_FILES['galaxy']}"
            f" --roles-path \"{constants.base_roles_path}\"",
        )
        step = f"RUN {env}distronode-galaxy collection install $DISTRONODE_GALAXY_CLI_COLLECTION_OPTS {install_opts}"
        self.steps.append(step)
    ...
    def _prepare_galaxy_copy_steps(self) -> None:
        if self.definition.get_dep_abs_path('galaxy'):
            dir_name = os.path.dirname(constants.base_collections_path.rstrip('/'))  # /usr/share/distronode
            self.steps.extend([
                "",
