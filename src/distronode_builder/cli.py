...
from .main import DistronodeBuilder
...
def run():
    ...
    if args.action in ['create', 'build']:
        ab = DistronodeBuilder(**vars(args))
...
def get_version():
    return importlib.metadata.version('distronode_builder')
...
def add_container_options(parser):
    ...
    build_command_parser.add_argument(
        ...
        help='Build-time variables to pass to any podman or docker calls. '
             f'Internally distronode-builder makes use of {", ".join(constants.build_arg_defaults.keys())}'
    )
    ...
    for p in [create_command_parser, build_command_parser]:
        ...
        p.add_argument('--galaxy-ignore-signature-status-codes',
                       ...
                       'distronode-galaxy. May be specified multiple times. See distronode-galaxy doc for more info.')
        p.add_argument('--galaxy-required-valid-signature-count',
                       ...
                       'distronode-galaxy ~if there are any signatures provided~. See distronode-galaxy doc for more info.')
...
def parse_args(args=None):
    ...
    parser = argparse.ArgumentParser(
        prog='distronode-builder',
        description=(
            'Tooling to help build container images for running Distronode content. '
            'Get started by looking at the help text for one of the subcommands.'
        )
    )
    parser.add_argument(
        '--version', action='version', version=get_version(),
        help='Print distronode-builder version and exit.'
    )
