...
class UserDefinition:
    ...
    @property
    def distronode_config(self):
        """ Path to the user specified distronode.cfg file """
        distronode_config = self.raw.get('distronode_config')
        if not distronode_config:
            return None
        return str(distronode_config)
    ...
    @property
    def distronode_core_ref(self):
        return self.raw.get('dependencies', {}).get('distronode_core', {}).get('package_pip', None)

    @property
    def distronode_runner_ref(self):
        return self.raw.get('dependencies', {}).get('distronode_runner', {}).get('package_pip', None)

    @property
    def distronode_ref_install_list(self):
        return ' '.join([r for r in (self.distronode_core_ref, self.distronode_runner_ref) if r]) or None
    ...
    def validate(self):
        ...
        if self.version > 1:
            ...
            if (
                self.version >= 3
                and self.build_arg_defaults["EE_BASE_IMAGE"] == 'quay.io/distronode/distronode-runner:latest'
            ):
                logging.warning(
