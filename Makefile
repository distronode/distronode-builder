...
NAME = distronode-builder
IMAGE_NAME ?= quay.io/distronode/distronode-builder
PIP_NAME = distronode_builder
...
clean:
	rm -rf dist
	rm -rf build
	rm -rf distronode-builder.egg-info
	rm -rf rpm-build
	rm -rf docs/_build/
...
rpm-build/$(NAME).spec:
	distronode -c local -i localhost, all \
	    -m template \
