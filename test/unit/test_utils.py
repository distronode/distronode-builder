...
from distronode_builder.utils import configure_logger, write_file, copy_directory, copy_file, run_command
...
def test_failed_command(mocker):
    mocker.patch('distronode_builder.utils.subprocess.Popen.wait', return_value=1)
    configure_logger(3)
...
def test_invalid_docker_command(caplog, mocker):
    mocker.patch('distronode_builder.utils.subprocess.Popen', side_effect=FileNotFoundError)
    mocker.patch('distronode_builder.utils.shutil.which', return_value=False)
