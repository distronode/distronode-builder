...
def test_definition_version(exec_env_definition_file):
    ...
    aee = DistronodeBuilder(action='create', filename=path)
...
def test_galaxy_requirements(exec_env_definition_file, galaxy_requirements_file, path_spec, tmp_path):
    ...
    aee = DistronodeBuilder(action='create', filename=exec_env_path, build_context=str(tmp_path / 'bc'))
    aee.build()
...
def test_base_image_via_build_args(exec_env_definition_file, tmp_path):
    ...
    aee = DistronodeBuilder(action='create', filename=path, build_context=tmp_path.joinpath('bc').as_posix())
    aee.build()

    with open(aee.containerfile.path) as f:
        content = f.read()

    assert 'distronode-runner' in content

    aee = DistronodeBuilder(
        action='create',
...
def test_base_image_via_definition_file_build_arg(exec_env_definition_file, tmp_path):
    ...
    path = exec_env_definition_file(content=content)
    aee = DistronodeBuilder(action='create', filename=path, build_context=tmp_path.joinpath('bc'))
    aee.build()
...
def test_build_command(exec_env_definition_file, runtime):
    ...
    aee = DistronodeBuilder(action='create', filename=path, tag=['my-custom-image'])
    command = aee.build_command
    assert 'build' in command
    assert 'my-custom-image' in command

    aee = DistronodeBuilder(action='create', filename=path, build_context='foo/bar/path', container_runtime=runtime)
...
def test_nested_galaxy_file(data_dir, tmp_path):
    ...
    DistronodeBuilder(action='create', filename=nested_galaxy_file, build_context=tmp_path).build()
...
def test_distronode_config_for_galaxy(exec_env_definition_file, tmp_path, data_dir):
    distronode_config_path = str(data_dir / 'distronode_cfg_for_galaxy' / 'distronode-test.cfg')
    galaxy_req = str(data_dir / 'distronode_cfg_for_galaxy' / 'requirements.yml')
    content = {
        'version': 1,
        'distronode_config': distronode_config_path,
        'dependencies': {
            'galaxy': galaxy_req,
        },
    }
    ...
    assert f'COPY {constants.user_content_subfolder}/distronode.cfg ~/.distronode.cfg' in content
