...
from distronode_builder.user_definition import UserDefinition
...
def test_prepare_galaxy_install_steps(build_dir_and_ee_yml):
    ...
    expected = [
        ...
        f"RUN DISTRONODE_GALAXY_DISABLE_GPG_VERIFY=1 distronode-galaxy collection install "
        f"$DISTRONODE_GALAXY_CLI_COLLECTION_OPTS -r {constants.CONTEXT_FILES['galaxy']} "
...
def test_prepare_galaxy_install_steps_with_keyring(build_dir_and_ee_yml):
    ...
    expected = [
        f"RUN distronode-galaxy role install $DISTRONODE_GALAXY_CLI_ROLE_OPTS "
        f"-r {constants.CONTEXT_FILES['galaxy']} --roles-path \"{constants.base_roles_path}\"",
        f"RUN distronode-galaxy collection install $DISTRONODE_GALAXY_CLI_COLLECTION_OPTS "
...
def test_v3_various(build_dir_and_ee_yml):
    ...
    assert "RUN /output/scripts/check_distronode $PYCMD" in c.steps
    ...
    assert "RUN $PYCMD -m pip install --no-cache-dir $DISTRONODE_INSTALL_REFS" in c.steps
...
def test__handle_additional_build_files(build_dir_and_ee_yml):
    ...
    assert (config_dir / 'distronode.cfg').exists()
