...
class TestUserDefinition:
    ...
    def test_definition_syntax_error(self, data_dir):
        ...
        with pytest.raises(DefinitionError) as error:
            DistronodeBuilder(action='create', filename=path)
    ...
    @pytest.mark.parametrize('yaml_text,expect', [
        ...
        (
            "{'version': 1, 'distronode_config': ['distronode.cfg']}",
            "['distronode.cfg'] is not of type 'string'"
        ),
        (
        ...
        (
            "{'version': 3, 'distronode_config': 'distronode.cfg' }",
            "Additional properties are not allowed ('distronode_config' was unexpected)"
        ),  # distronode_config not supported in v3
        (
            "{'version': 3, 'images': { 'base_image': {'name': 'base_image:latest'}, "
        ...
        (
            "{'version': 3, 'options': { 'skip_distronode_check': 'True' } }",
        ...
        'dest_has_dot_dot', 'dest_is_absolute', 'src_req', 'dest_req', 'distronode_cfg',
    ...
    def test_ee_validated_early(self, exec_env_definition_file):
        """
        Expect the EE file to be validated early during DistronodeBuilder instantiation.
        """
    ...
    def test_v3_distronode_install_refs(self, exec_env_definition_file):
        path = exec_env_definition_file(
            """
            {'version': 3,
             'images': { 'base_image': {'name': 'base_image:latest'}},
             'dependencies': {
                'distronode_core': {'package_pip': 'distronode-core==2.13'},
                'distronode_runner': { 'package_pip': 'distronode-runner==2.3.1'}
             }
            }
            """
        )
        definition = UserDefinition(path)
        definition.validate()
        assert definition.distronode_core_ref == "distronode-core==2.13"
        assert definition.distronode_runner_ref == "distronode-runner==2.3.1"
        assert definition.distronode_ref_install_list == "distronode-core==2.13 distronode-runner==2.3.1"
    ...
    def test_v3_skip_distronode_check_default(self, exec_env_definition_file):
        """
        Test that options.skip_distronode_check defaults to False
        """
        path = exec_env_definition_file(
            "{'version': 3, 'images': { 'base_image': {'name': 'base_image:latest'}}}"
        )
        ...
        value = definition.raw.get('options', {}).get('skip_distronode_check')
...
class TestImageDescription:
    ...
    @pytest.mark.parametrize('key', ['base_image', 'builder_image'])
    @pytest.mark.parametrize('image', ['registry.redhat.io/distronode-automation-platform-21/ee-minimal-rhel8',
                                       'registry.redhat.io/distronode-automation-platform-21/ee-minimal-rhel8:'
                                       ])
