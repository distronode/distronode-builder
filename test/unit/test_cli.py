...
from distronode_builder.cli import parse_args
from distronode_builder.policies import PolicyChoices


def prepare(args):
    args = parse_args(args)
    return DistronodeBuilder(**vars(args))
...
def test_custom_distronode_galaxy_cli_collection_opts(exec_env_definition_file, tmp_path):
    content = {'version': 1}
    path = str(exec_env_definition_file(content=content))

    aee = prepare(['build', '-f', path, '--build-arg', 'DISTRONODE_GALAXY_CLI_COLLECTION_OPTS=--pre', '-c', str(tmp_path)])
    assert aee.build_args == {'DISTRONODE_GALAXY_CLI_COLLECTION_OPTS': '--pre'}


def test_custom_distronode_galaxy_cli_role_opts(exec_env_definition_file, tmp_path):
    content = {'version': 1}
    path = str(exec_env_definition_file(content=content))
    ...
    aee = prepare(
        [
            ...
            'DISTRONODE_GALAXY_CLI_ROLE_OPTS=--ignore-errors', '-c', str(tmp_path)
        ]
    )
    assert aee.build_args == {'DISTRONODE_GALAXY_CLI_ROLE_OPTS': '--ignore-errors'}
...
def test_build_args_empty_value(exec_env_definition_file, tmp_path):
    ...
    aee = prepare(['build', '-f', path, '--build-arg', 'DISTRONODE_GALAXY_CLI_ROLE_OPTS=', '-c', str(tmp_path)])
    assert aee.build_args == {'DISTRONODE_GALAXY_CLI_ROLE_OPTS': ''}
...
def test_build_args_no_trailing_equal(exec_env_definition_file, tmp_path):
    ...
    aee = prepare(['build', '-f', path, '--build-arg', 'DISTRONODE_GALAXY_CLI_ROLE_OPTS', '-c', str(tmp_path)])
    assert aee.build_args == {'DISTRONODE_GALAXY_CLI_ROLE_OPTS': None}
...
def test_build_args_multiple_equal_sign_value(exec_env_definition_file, tmp_path):
    ...
    aee = prepare(['build', '-f', path,
                   '--build-arg', 'DISTRONODE_GALAXY_CLI_ROLE_OPTS=',
...
def test_as_executable_module(capsys):
    """
    Test __main__ shim as if invoked by `python -m distronode_builder`
    """
    with pytest.raises(SystemExit) as sysexit:
        runpy.run_module('distronode_builder', run_name='__main__', alter_sys=True)
