...
def test_introspect_write(cli, data_dir):
    r = cli(f'distronode-builder introspect {data_dir}')
...
def test_introspect_with_sanitize(cli, data_dir):
    r = cli(f'distronode-builder introspect --sanitize {data_dir}')
    data = yaml.safe_load(r.stdout)  # assure that output is valid YAML
    assert 'python' in data
    assert 'system' in data
...
def test_introspect_write_bindep(cli, data_dir, tmp_path):
    ...
    cli(f'distronode-builder introspect {data_dir} --write-bindep={dest_file}')
...
def test_introspect_write_python(cli, data_dir, tmp_path):
    dest_file = tmp_path / 'req.txt'
    cli(f'distronode-builder introspect {data_dir} --write-pip={dest_file}')
...
def test_introspect_write_python_and_sanitize(cli, data_dir, tmp_path):
    dest_file = tmp_path / 'req.txt'
    cli(f'distronode-builder introspect {data_dir} --write-pip={dest_file} --sanitize')
...
def test_introspect_with_sanitize_user_reqs(cli, data_dir, tmp_path):
    ...
    user_file.write_text("distronode\npytest\n")

    r = cli(f'distronode-builder introspect --sanitize --user-pip={user_file} {data_dir}')
    data = yaml.safe_load(r.stdout)  # assure that output is valid YAML
    assert 'python' in data
    assert 'system' in data
    assert 'pytz  # from collection test.reqfile' in data['python']

    # 'distronode' allowed in user requirements
    assert 'distronode  # from collection user' in data['python']
