...
def test_version_1_warning(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    r = cli(
        f"distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} --container-runtime {runtime} -v3",
        allow_error=True
    )
...
def test_build_fail_exitcode(cli, runtime, ee_tag, tmp_path, data_dir):
    """Test that when a build fails, the distronode-builder exits with non-zero exit code.

    Example: https://github.com/distronode/distronode-builder/issues/51
    """
    bc = tmp_path
...
def test_blank_execution_environment(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    cli(
        f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} --container-runtime {runtime}'
    )
...
def test_multiple_tags(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    ee_def = data_dir / 'minimal_fast' / 'execution-environment.yml'
    cli(
        f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} -t testmultitags --container-runtime {runtime}'
    )
...
def test_user_system_requirement(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    command = f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} --container-runtime {runtime}'
    cli(command)
    result = cli(
...
def test_collection_system_requirement(cli, runtime, ee_tag, tmp_path, data_dir):
    bc = tmp_path
    ee_def = data_dir / 'distronode.posix.at' / 'execution-environment.yml'
    cli(
        f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} --container-runtime {runtime} -v3'
    )
    result = cli(
...
def test_user_python_requirement(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    assert 'The official command line interface for Distronode AWX' in result.stdout, result.stdout
...
def test_build_args_basic(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    result = cli(
        f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} '
...
def test_base_image_build_arg(cli, runtime, ee_tag, tmp_path, data_dir):
    ...
    cli(f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} --container-runtime {runtime} -v3')
    cli(f'distronode-builder build -c {bc} -f {ee_def} -t {ee_tag} '
...
def test_has_pytz(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    cli(
        f'distronode-builder build -c {tmp_path} -f {ee_def} -t {ee_tag} '
...
def test_build_layer_reuse(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    build_cmd = f'distronode-builder build -c {tmp_path} -f {ee_def} -t {ee_tag} --container-runtime {runtime} -v 3'
...
def test_collection_verification_off(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    #        aliasing `distronode-galaxy` to `/bin/true`
    ee_def = data_dir / 'pytz' / 'execution-environment.yml'
    result = cli(
        f'distronode-builder build --no-cache -c {tmp_path} -f {ee_def} -t {ee_tag} '
        f'--container-runtime {runtime} -v 3'
    )
    assert "RUN DISTRONODE_GALAXY_DISABLE_GPG_VERIFY=1 distronode-galaxy" in result.stdout
...
def test_collection_verification_on(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    # distronode-galaxy might error (older Distronode), but that should be ok
    result = cli(
        f'distronode-builder build --no-cache --galaxy-keyring {keyring} -c {tmp_path} '
    ...
    assert "RUN DISTRONODE_GALAXY_DISABLE_GPG_VERIFY=1 distronode-galaxy" not in result.stdout
...
def test_galaxy_signing_extra_args(cli, runtime, data_dir, ee_tag, tmp_path):
    ...
    result = cli(f'distronode-builder build --no-cache -c {tmp_path} -f {ee_def} -t {ee_tag} '
                 f'--container-runtime {runtime} -v 3 '
