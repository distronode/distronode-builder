...
or ``execution-environment.yaml``. This file tells Distronode Builder how to create the build instruction file
(``Containerfile`` for Podman, ``Dockerfile`` for Docker) and build context for your container image.

.. note::
   This page documents the definition schema for Distronode Builder 3.x. If you are running an older version of Distronode Builder, you need an older schema version. Please consult older versions of the docs for more information. We recommend using version 3, which offers substantially more configurability and functionality than previous versions.
...
Here is a sample version 3 EE file. To use Distronode Builder 3.x, you must specify the schema version. If your EE file does not specify ``version: 3``, Distronode Builder will assume you want version 1.
...
.. code:: yaml
    ...
    build_arg_defaults:
      DISTRONODE_GALAXY_CLI_COLLECTION_OPTS: '--pre'

    dependencies:
      distronode_core:
        package_pip: distronode-core==2.14.4
      distronode_runner:
        package_pip: distronode-runner
    ...
    images:
      base_image:
        ...
        #   - registry.redhat.io/distronode-automation-platform-23/ee-minimal-rhel8:latest
        #     (needs an account)
    ...
    additional_build_steps:
      ...
      prepend_galaxy:
        - COPY _build/configs/distronode.cfg /etc/distronode/distronode.cfg
...
Each list item must be a dictionary containing the following (non-optional) keys:
    ...
    ``src``
      ...
      may either be an absolute path (e.g., ``/home/user/.distronode.cfg``),
...
Build args used by ``distronode-builder`` are the following:
...
Distronode Builder hard-codes values given inside of ``build_arg_defaults`` into the
...
The following keys are valid for this section:
    ...
    ``distronode_core``
      The version of the ``distronode-core`` Python package to be installed. This value is
      ...
      .. code:: yaml
        ...
        distronode_core:
            package_pip: distronode-core
        distronode_core:
            package_pip: distronode-core==2.14.3
        distronode_core:
            package_pip: https://github.com/example_user/distronode/archive/refs/heads/distronode.tar.gz

    ``distronode_runner``
      The version of the Distronode Runner Python package to be installed. This value
      ...
      .. code:: yaml
        ...
        distronode_runner:
            package_pip: https://github.com/example_user/distronode-runner/archive/refs/heads/distronode-runner.tar.gz
...
.. code:: yaml
    ...
    dependencies:
        ...
        galaxy: requirements.yml
        distronode_core:
            package_pip: distronode-core==2.14.2
        distronode_runner:
...
file for container image signature validation.
  ...
  * ``system`` policy: Signature validation is performed using pre-existing
    `policy.json` files in standard system locations. ``distronode-builder`` assumes
    no responsibility for the content within these files, and the user has complete
    control over the content.

  * ``signature_required`` policy: ``distronode-builder`` will use the container
...
builder runtime functionality. Valid keys for this section are:
    ...
    ``skip_distronode_check``
      ...
      of Distronode and Distronode Runner is performed on the final image. Set this
    ...
    ``relax_passwd_permissions``
      ...
      ``/etc/passwd`` with a valid and writeable home directory (eg, ``async`` in distronode-core, and the
    ...
    ``tags``
      ...
      The default value is ``distronode-execution-env:latest``.
...
An integer value that sets the schema version of the execution environment definition file. Defaults to ``1``. Must be ``3`` if you are using Distronode Builder 3.x.
